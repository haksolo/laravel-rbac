...
@section('content')
<div class='container'>
  <form action="{{ route('role-bindings.store') }}" method='POST'>
    @csrf
    ...
    <div class='form-group'>
      ...
      <select id='role_kind' name='role_kind' class="custom-select{{ $errors->has('role_kind') ? ' is-invalid' : '' }}">
        <option value='role' {{ old('role_kind') == 'role' ? 'selected' : '' }}>Role</option>
      </select>
      @error('role_kind')
      <div class='invalid-feedback'>{{ $message }}</div>
      @enderror
    ...
    <div class='form-group'>
      ...
      <select id='role_name' name='role_name' class="custom-select{{ $errors->has('role_name') ? ' is-invalid' : '' }}">
        ...
        <option value='{{ $option->name }}' {{ old('role_name') == $option->name ? 'selected' : '' }}>{{ $option->name }}</option>
        @endforeach
      </select>
      @error('role_name')
      <div class='invalid-feedback'>{{ $message }}</div>
